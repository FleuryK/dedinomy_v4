	...
	protected function parseForeach(&$c){
		for($i=0; $i<10 && strpos($c, '</'.preg_quote($this->bal['foreach'][0])) !== FALSE; $i++){
			$c = preg_replace_callback(
				'`<('.preg_quote($this->bal['foreach'][0]).')\s*'.preg_quote($this->bal['foreach'][1]).'="(.+)"\s*'.preg_quote($this->bal['foreach'][2]).'="(.+)"\s*('.preg_quote($this->bal['foreach'][3]).'=".+")?\s*>((?:.(?!<'.preg_quote($this->bal['foreach'][0]).'>))*)</'.preg_quote($this->bal['foreach'][0]).'>`sU',
