...
class Paginate
{
	public static function numbers($url, $link, $total, $current, $adj=3, $options='')
	{
	    ...
	    if($total > 1)
	    {
	        ...
	        if ($total < 7 + ($adj * 2))
	        {
	            ...
	            $pagination .= ($current == 1) ? '<span class="active">1</span>' : "<a href=\"{$url}{$options}\">1</a>"; // Opérateur ternaire : (condition) ? 'valeur si vrai' : 'valeur si fausse'
	            ...
	            for ($i=2; $i<=$total; $i++)
	            {
	                ...
	                else
	                {
	                    ...
	                    $pagination .= "<a href=\"{$url}{$link}{$i}{$options}\">{$i}</a>";
	                }
	            }
	        }
	        /* =========================================
	        ...
	        else
	        {
	            ...
	            if ($current < 2 + ($adj * 2))
	            {
	                ...
	                $pagination .= ($current == 1) ? "<span class=\"active\">1</span>" : "<a href=\"{$url}{$options}\">1</a>";
	                ...
	                $pagination .= "<a href=\"{$url}{$link}{$penultimate}{$options}\">{$penultimate}</a>";
	                $pagination .= "<a href=\"{$url}{$link}{$total}{$options}\">{$total}</a>";
	            }
	            /**
	            ...
	            elseif((($adj * 2) + 1 < $current) && ($current < $total - ($adj * 2)))
	            {
	                ...
	                $pagination .= "<a href=\"{$url}{$options}\">1</a>";
	                $pagination .= "<a href=\"{$url}{$link}2{$options}\">2</a>";
	                $pagination .= '&hellip;';
	        ...
	        {
	            $pagination .= "<a href=\"{$url}{$link}{$next}{$options}\">►</a>\n";
	        }
